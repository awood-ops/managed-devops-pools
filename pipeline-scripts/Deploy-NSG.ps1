param (
  ...
  [Parameter()]
  [String]$TemplateFile = "upstream-releases\$($version)\infra-as-code\bicep\modules\nsg.bicep",

  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\nsg.parameters.bicepparam",
...
$inputObject = @{
    DeploymentName        = 'NSG-Deploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
