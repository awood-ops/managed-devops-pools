param (
  ...
  [Parameter()]
  [String]$TemplateFile = "upstream-releases\$($version)\infra-as-code\bicep\modules\keyVault.bicep",

  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\keyVault.parameters.bicepparam",
...
$inputObject = @{
    DeploymentName        = 'KeyVaultDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
