param (
  ...
  [Parameter()]
  [String]$TemplateFile = "upstream-releases\$($version)\infra-as-code\bicep\modules\vNet.bicep",

  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\vNet.parameters.bicepparam",
...
$inputObject = @{
    DeploymentName        = 'NetworkingDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
