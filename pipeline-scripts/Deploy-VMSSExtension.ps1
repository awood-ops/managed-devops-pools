param (
  ...
  [Parameter()]
  [String]$TemplateFile = "upstream-releases\$($version)\infra-as-code\bicep\modules\vmssExtension.bicep",

  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\vmssExtension.parameters.bicepparam",
...
$inputObject = @{
    DeploymentName        = 'VMSSExtensionDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
