param (
  ...
  [Parameter()]
  [String]$TemplateFile = "upstream-releases\$($version)\infra-as-code\bicep\modules\vmss.bicep",

  [Parameter()]
  [String]$TemplateParameterFile = "config\custom-parameters\vmss.parameters.bicepparam",
...
$inputObject = @{
    DeploymentName        = 'VMSSDeploy-{0}' -f ( -join (Get-Date -Format 'yyyyMMddTHHMMssffffZ')[0..63])
